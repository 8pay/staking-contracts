...
const StakePoolWithLockPeriod = artifacts.require('StakePoolWithLockPeriod');
const MockToken = artifacts.require('MockToken');

contract('StakePoolWithLockPeriod', accounts => {
  const [owner, bob, alice, nonOwner] = accounts;
  const rewardPerBlock = new BN('2000');
  const lockPeriod = time.duration.days(60);
  ...
  beforeEach(async () => {
    ...
    this.pool = await StakePoolWithLockPeriod.new(
  ...
  it('should update lock period', async () => {
    const newLockPeriod = time.duration.days(3);
    await this.pool.setLockPeriod(newLockPeriod, { from: owner });
    const updatedLockPeriod = await this.pool.lockPeriod();
    expect(updatedLockPeriod).to.be.bignumber.equal(newLockPeriod);
  });

  it('reverts when updating lock period from non-owner', async () => {
    const newLockPeriod = time.duration.days(3);

    await expectRevert(
      this.pool.setLockPeriod(newLockPeriod, { from: bob }),
  ...
  describe('after adding reward', () => {
    ...
    describe('after startBlock', () => {
      ...
      describe('after deposit', () => {
        ...
        it('reverts when withdrawing tokens before lock period is over', async () => {
          await expectRevert(
            this.pool.withdraw(this.amount, { from: bob }),
            'Pool: cannot withdraw before lock period is over'
          );
        });
        ...
        describe('after lock period is over', () => {
          beforeEach(async () => {
